...
import { ExternalLink, Smartphone, Zap, AlertCircle } from "lucide-react"; // Añadí AlertCircle

// Íconos SVG de redes sociales y stores como componentes simples (se mantienen igual)
const InstagramIcon = () => (
...
export default function Footer() {
  return (
    <footer className="relative mt-16 border-t border-border bg-card pb-20 lg:pb-0">
      ...
      {/* Cambiado a max-w-7xl y lg:grid-cols-4 para acomodar la nueva columna */}
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-6">
              ...
              <div className="flex items-center gap-2">
                {[
                  ...
                  <Link
                    ...
                    title={label}
                    style={
                      { backgroundColor: brandColor } as React.CSSProperties
                    }
                    className="w-8 h-8 rounded-lg flex items-center justify-center text-white transition-all duration-200 hover:opacity-80"
                  >
                    {icon}
          ...
          {/* ── Columna 3: Legal y Soporte (NUEVA) ── */}
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-foreground flex items-center gap-2 uppercase tracking-wider">
            ...
            <ul className="space-y-3 text-sm text-muted-foreground">
              ...
              <li className="pt-2">
                <Link
                  href="/reportar"
                  className="inline-flex items-center gap-1.5 text-orange-500/90 hover:text-orange-600 transition-colors font-medium group"
                >
                  <AlertCircle className="w-4 h-4" />
                  <span>Reportar error o dato</span>
                </Link>
              </li>
            </ul>
          </div>
